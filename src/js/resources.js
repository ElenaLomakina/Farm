var dom = require("./lib/dom");
    ...
    GardenResource.prototype.infoContent = function () {
        ...
        var content = "";
        if (this.infoBlock.classList.contains("empty")) {
            content = "one may plant here";
        }
        else if (this.infoBlock.classList.contains("ripening")) {
            content = fruits + " are ripening";
        }
        else if (this.infoBlock.classList.contains("ripe")) {
            content = fruits + " have already ripened";
        }
        else if (this.infoBlock.classList.contains("depleted")) {
            content = "the ground should be restored";
        }
        return content;
    };
    ...
    GardenResource.prototype.createBlock = function () {
        var block = dom.create(field, "div", ["resource", this.product.name]);
        dom.blockContent(block, "", this.product.color);
        var title = dom.create(block, "div", ["resource-title"]);
        var type = dom.create(title, "h4", ["resource-type"]);
        dom.blockContent(type, this.type);
        var name = dom.create(title, "h4", ["resource-name"]);
        dom.blockContent(name, this.product.name);
        var info = dom.create(block, "div", ["resource-info", "empty"]);
        dom.blockContent(info, "one may plant here");
        this.infoBlock = info;
    };

module.exports.GardenResource = GardenResource;